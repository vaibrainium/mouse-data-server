...
def safe_time_parse(time_str: str):
...
def get_sensory_noise(data: pd.DataFrame):
	...
	return 1 / sensitivity if sensitivity else np.nan
...
def get_recent_sessions(last_X_business_days=None, start_date=None, end_date=None) -> pd.DataFrame:
	...
	session_data = []
	cols = ["date", "start_weight", "end_weight", "baseline_weight", "protocol", "experiment", "session", "session_uuid"]
	...
	for mouse in mouse_ids:
		...
		history = pd.read_csv(history_path, parse_dates=["date"], dayfirst=True)
		...
		history = history.reindex(columns=cols, fill_value=pd.NA)
	...
	for mouse_id in new_sessions.mouse_id.unique():
		...
		for date in mouse_sessions.date.unique():
			...
			for _, meta in sessions.iterrows():
				...
				start_time = summary_row.start_time.values[0] if not summary_row.empty else None
				end_time = summary_row.end_time.values[0] if not summary_row.empty else None
				...
				new_sessions.loc[condition, [
					...
					safe_time_parse(start_time),
					safe_time_parse(end_time),
					max(valid_trials.idx_attempt, default=np.nan),
					max(valid_trials.idx_valid, default=np.nan),
					safe_nanmean(valid_trials.outcome) * 100,
					safe_nanmean(valid_trials.outcome[np.sign(valid_trials.signed_coherence) == -1]) * 100,
					safe_nanmean(valid_trials.outcome[np.sign(valid_trials.signed_coherence) == 1]) * 100,
					int(np.nansum(valid_trials.trial_reward)),
					get_sensory_noise(all_trials),
