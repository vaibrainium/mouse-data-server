...
def safe_time_parse(time_str: str, formatted: bool = False):
...
def get_recent_sessions(last_X_business_days=None, start_date=None, end_date=None) -> pd.DataFrame:
	...
	for mouse in mouse_ids:
		...
		for i, row in raw_history.iterrows():
			try:
				row_data = row.reindex(required_cols, fill_value=pd.NA)
...
def process_rdk_analysis(meta, new_sessions, analyzed_data):
...
def process_basic_analysis(meta, new_sessions, analyzed_data):
		...
		for date in mouse_sessions.date.unique():
			...
			for _, meta in sessions.iterrows():
				...
				if meta.experiment in ["rt_directional_training", "rt_maintenance", "rt_test", "rt_dynamic_training"]:
					process_rdk_analysis(meta, new_sessions, analyzed_data)
				elif meta.experiment in ["free_reward_training", "reward_spout_association"]:
					process_basic_analysis(meta, new_sessions, analyzed_data)
				else:
